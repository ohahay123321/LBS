...
<body>
    ...
    <div class="sidebar">
        <div class="sidebar-profile">
            ...
            <p>Welcome Admin!</p>
            <h3>{{ $admin->name ?? explode('@', $admin->email)[0] }}</h3>
        </div>
        ...
        <div class="topbar">
            ...
            <div class="topbar-right">
                ...
                <div class="user-pill" onclick="showTab('profile'); return false;">
                    <img src="{{ $admin->profile_image_url }}" width="32" height="32" style="object-fit: cover;">
                    <span>{{ $admin->name ?? explode('@', $admin->email)[0] }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="content">
            ...
            <div id="dash" class="tab-content active">
                ...
                <div class="dashboard-grid">
                    <div class="dash-card card-primary" onclick="showTab('manage_users')">
                        <div><h2>{{ $stats['members'] }}</h2><p>Members</p></div>
                    ...
                    <div class="dash-card card-danger" onclick="showTab('issued_books')">
                        <div><h2>{{ $stats['issued'] }}</h2><p>Issued Books</p></div>
                    ...
                    <div class="dash-card card-success" onclick="showTab('view_books')">
                        <div><h2>{{ $stats['available'] }}</h2><p>Available Books</p></div>
                    ...
                    <div class="dash-card card-warning" onclick="showTab('fines')">
                        <div><h2>₱{{ number_format($stats['total_fine'], 2) }}</h2><p>Fines</p></div>
                    ...
                    <div class="dash-card card-warning" onclick="showTab('req_books')">
                        <div><h3>Requested Books</h3><p>{{ $stats['pending'] }} Pending Requests</p></div>
            ...
            <div id="profile" class="tab-content">
                ...
                <div class="profile-container">
                    <div class="profile-left">
                        <div class="profile-img-box"><img src="{{ $admin->profile_image_url }}" alt="Profile"></div>
                        <form method="POST" action="{{ route('admin.profile.image') }}" enctype="multipart/form-data" class="upload-form">
                            @csrf
                    ...
                    <div class="profile-right">
                        <form method="POST" action="{{ route('admin.profile.update') }}">
                            @csrf
                            <div class="input-group"><label>Name:</label><input type="text" name="name" value="{{ $admin->name ?? '' }}" placeholder="Enter your name"></div>
                            <div class="input-group"><label>Email (Read Only):</label><input type="email" value="{{ $admin->email }}" readonly></div>
                            <div class="input-group"><label>Phone No:</label><input type="text" name="phone" value="{{ $admin->phone ?? '' }}" placeholder="Enter phone number"></div>
                        <div class="input-group"><label>Address:</label><textarea name="address" rows="4" placeholder="Enter full address">{{ $admin->address ?? '' }}</textarea></div>
                ...
                <div id="add_f" style="display:none;" class="add-form">
                    <form method="POST" action="{{ route('admin.books.store') }}" enctype="multipart/form-data">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                            ...
                            <div class="input-group" style="margin-bottom: 0;"><label>Category</label><select name="category"><option value="" disabled selected>-- Select Category --</option>@foreach($categories as $c)<option>{{ $c->name }}</option>@endforeach</select></div>
            ...
            <div id="req_books" class="tab-content">
                ...
                <table>
                    ...
                    <tr>
                        <td><strong>{{ $row->student_name }}</strong><br><span style="color: var(--text-muted); font-size: 13px;">ID: {{ $row->student_id_num }}</span></td>
                        <td><strong>{{ $row->book->title ?? 'N/A' }}</strong></td>
                        <td>{{ $row->req_date }}</td>
                        <td>
                            <form method="POST" action="{{ route('admin.requests.approve', $row->id) }}" style="display:inline;">@csrf<button type="submit" class="btn-success btn-sm" style="margin-right: 8px;">Approve</button></form>
                            <form method="POST" action="{{ route('admin.requests.deny', $row->id) }}" style="display:inline;">@csrf<button type="submit" class="btn-danger btn-sm">Deny</button></form>
                        </td>
                    </tr>
                    @empty
            ...
            <div id="manage_users" class="tab-content">
                ...
                <table>
                    ...
                    @forelse($admins as $u)
                    <tr>
                        <td>{{ $u->id }}</td><td>{{ $u->email }}</td><td>{{ $u->name ?? '-' }}</td><td>{{ $u->email_verified ? 'Yes' : 'No' }}</td>
                        <td>
                            @if($u->id != $admin->id)
                            <form method="POST" action="{{ route('admin.users.destroy', $u->id) }}" style="display:inline;" onsubmit="return confirm('Delete this admin?')">
                                @csrf @method('DELETE')
                ...
                <table>
                    ...
                    <tr>
                        <td>{{ $u->id }}</td><td>{{ $u->email }}</td><td>{{ $u->name ?? '-' }}</td><td>{{ $u->email_verified ? 'Yes' : 'No' }}</td>
                        <td>
                            <form method="POST" action="{{ route('admin.users.destroy', $u->id) }}" style="display:inline;" onsubmit="return confirm('Delete this student?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn-danger btn-sm">Delete</button>
                            </form>
                        </td>
            ...
            <div id="system_logs" class="tab-content">
                ...
                <table>
                    ...
                    <tr><td>{{ $l->timestamp }}</td><td>{{ $l->description }}</td></tr>
                    @empty
            ...
            <div id="issued_books" class="tab-content">
                ...
                <table>
                    ...
                    @forelse($issuedBooks as $row)
                    <tr>
                        <td><strong>{{ $row->student_name }}</strong><br><span style="color: var(--text-muted); font-size: 13px;">ID: {{ $row->student_id_num }}</span></td>
                        <td><strong>{{ $row->book->title ?? 'N/A' }}</strong></td>
                        <td>{{ $row->action_date }}</td>
                        <td><strong style="color: var(--danger);">{{ $row->return_date }}</strong></td>
                        <td>
                            <form method="POST" action="{{ route('admin.requests.return', $row->id) }}" style="display:inline;">@csrf<button type="submit" class="btn-success btn-sm">Mark Returned</button></form>
                        </td>
                    </tr>
                    @empty
            ...
            <div id="fines" class="tab-content">
                ...
                <div class="add-form" style="margin-bottom: 30px;">
                    <form method="POST" action="{{ route('admin.fine-rate') }}" style="display: flex; gap: 12px; align-items: flex-end;">
                        @csrf
                        <div class="input-group" style="margin-bottom: 0; flex: 1;"><label>Fine Rate (PHP per day)</label><input type="number" name="fine_rate" value="{{ $stats['fine_rate'] }}" min="1" required></div>
                ...
                <table>
                    ...
                    @forelse($fines as $f)
                    <tr>
                        <td><strong>{{ $f->user->name ?? $f->student_name }}</strong><br><span style="color: var(--text-muted); font-size: 13px;">{{ $f->user->email ?? '' }}</span></td>
                        <td><strong>{{ $f->book->title ?? 'N/A' }}</strong></td>
                        <td>{{ $f->return_date }}</td>
                        <td><span style="color: var(--danger);">{{ $f->days_late ?? 0 }} days</span></td>
                        <td><strong style="color: var(--warning);">₱{{ number_format($f->fine, 2) }}</strong></td>
                        <td>{!! $f->fine_paid ? '<span class="status-RETURNED">Paid</span>' : '<span class="status-BORROWED">Unpaid</span>' !!}</td>
                        <td>@if(!$f->fine_paid)<form method="POST" action="{{ route('admin.requests.pay-fine', $f->id) }}" style="display:inline;">@csrf<button type="submit" class="btn-success btn-sm">Mark Paid</button></form>@endif</td>
                    </tr>
                    @empty
    ...
    <script>
                ...
                .then(d => {
                    ...
                    if (d.notifications.length === 0) {
                        ...
                        list.innerHTML = d.notifications.map(n => {
                            let icon;
                            if (n.data.type === 'new_request') {
                                icon = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line></svg>';
                            } else if (n.data.type === 'request_denied') {
                                ...
                                icon = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>';
                            } else {
        ...
        function markAllRead() {
            fetch('{{ url("admin/notifications/read-all") }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } })
                .then(() => { fetchNotifs(); });
        }
        ...
        window.addEventListener('load', function() {
            ...
            if (typeof Chart !== 'undefined') {
                new Chart(document.getElementById('booksStatusChart').getContext('2d'), {
                    type: 'doughnut',
                    data: {
                        labels: @json($booksByStatus['labels']),
                        datasets: [{ data: @json($booksByStatus['data']), backgroundColor: ['#10b981', '#ef4444'], borderWidth: 0 }]
                    },
                ...
                new Chart(document.getElementById('requestsChart').getContext('2d'), {
                    ...
                    data: {
                        ...
                        datasets: [{ label: 'Requests', data: @json($requestsByMonth), borderColor: '#2563eb', backgroundColor: 'rgba(37, 99, 235, 0.1)', fill: true, tension: 0.4 }]
                    },
                ...
                new Chart(document.getElementById('finesChart').getContext('2d'), {
                    ...
                    data: {
                        labels: @json($monthLabels),
                        datasets: [{ label: 'Fines (PHP)', data: @json($finesByMonth), backgroundColor: '#f59e0b', borderRadius: 6 }]
                    },
                ...
                new Chart(document.getElementById('categoriesChart').getContext('2d'), {
                    ...
                    data: {
                        ...
                        datasets: [{ label: 'Books', data: @json($booksByCategoryData), backgroundColor: '#3b82f6', borderRadius: 6 }]
                    },
                    options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } }
                });
            }
        ...
        <div id="edit_f" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:200; justify-content:center; align-items:center;">
            <div style="background:#fff; padding:32px; border-radius:12px; width:500px; max-width:90%;">
                ...
                <form method="POST" action="{{ route('admin.books.update') }}" enctype="multipart/form-data">
                    ...
                    <div class="input-group"><label>Category</label><select name="category" id="edit_category">@foreach($categories as $c)<option>{{ $c->name }}</option>@endforeach</select></div>

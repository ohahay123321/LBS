...
<body>
    ...
    <div class="sidebar">
        <div class="sidebar-profile">
            <img src="<?php echo e(asset('imagess.png')); ?>" alt="Profile">
            <p>Welcome Student!</p>
            <h3><?php echo e(explode('@', Auth::guard('student')->user()->email)[0]); ?></h3>
        </div>
        <form method="POST" action="<?php echo e(route('student.logout')); ?>" style="display:contents;">
            <?php echo csrf_field(); ?>
    ...
    <div class="content">
        ...
        <form method="get" class="search-bar">
            <input type="text" name="search" placeholder="Search by title, author, or ISBN..." value="<?php echo e($search); ?>">
            <input type="hidden" name="filter" value="<?php echo e($filter); ?>">
            <button type="submit">Search</button>
        </form>

        <div class="filter-buttons">
            <a href="?filter=available&search=<?php echo e($search); ?>&category=<?php echo e($category); ?>" class="filter-btn <?php echo e($filter == 'available' ? 'active' : ''); ?>">Available Books</a>
            <a href="?filter=borrowed&search=<?php echo e($search); ?>&category=<?php echo e($category); ?>" class="filter-btn borrowed <?php echo e($filter == 'borrowed' ? 'active' : ''); ?>">Borrowed Books</a>
            <a href="?filter=outofstock&search=<?php echo e($search); ?>&category=<?php echo e($category); ?>" class="filter-btn outofstock <?php echo e($filter == 'outofstock' ? 'active' : ''); ?>">Out of Stock</a>
        </div>

        <div style="margin-bottom: 30px;">
            <div class="section-title">Categories</div>
            <div class="filter-buttons">
                <a href="?filter=<?php echo e($filter); ?>&search=<?php echo e($search); ?>" class="filter-btn <?php echo e(empty($category) ? 'active' : ''); ?>">All</a>
                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="?filter=<?php echo e($filter); ?>&search=<?php echo e($search); ?>&category=<?php echo e($cat->name); ?>" class="filter-btn <?php echo e($category == $cat->name ? 'active' : ''); ?>"><?php echo e($cat->name); ?></a>
        ...
        <div class="books-grid">
            ...
            <div class="book-card">
                <div class="book-card-image">
                    ...
                    <div class="status-<?php echo e($b->status); ?>"><?php echo e($b->status); ?></div>
                </div>
                <div class="book-card-body">
                    ...
                    <?php if($b->status == 'AVAILABLE' && ($b->stock ?? 1) > 0): ?>
                    <form method="POST" action="<?php echo e(route('student.request')); ?>" class="request-form">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="book_id" value="<?php echo e($b->id); ?>">
                        <input type="hidden" name="student_name" value="<?php echo e(Auth::guard('student')->user()->name); ?>">
                        <input type="hidden" name="student_id_num" value="<?php echo e(Auth::guard('student')->user()->student_number); ?>">
                        <button type="submit">Request Book</button>
                    </form>
                    <?php elseif($b->status == 'BORROWED'): ?>
                    <div class="notice-box">Currently Borrowed</div>
                    <?php elseif(($b->stock ?? 1) == 0): ?>
        ...
        <table>
            ...
            <?php $__empty_1 = true; $__currentLoopData = $myRequests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $h): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr>
                <td><strong><?php echo e($h->book->title ?? 'N/A'); ?></strong></td>
                <td class="status-<?php echo e($h->status); ?>"><?php echo e($h->status); ?></td>
                <td><?php echo e($h->req_date); ?></td>
                <td>
                    <?php if($h->status == 'APPROVED'): ?>
                    <span style="color: var(--success); font-weight: 600;">Accepted on:</span> <?php echo e($h->action_date); ?><br>
                    <span style="color: var(--danger); font-weight: 600;">Return by:</span> <?php echo e($h->return_date); ?><br>
                    <span style="color: var(--text-muted); font-size: 13px;">Approved by: <?php echo e($h->admin->name ?? 'Unknown'); ?></span><br>
                    <span style="color: var(--text-muted); font-size: 13px;"><?php echo e($h->student_name); ?> Approved by <?php echo e($h->admin->name ?? 'Unknown'); ?></span>
                    <?php elseif($h->status == 'DENIED'): ?>
                    <span style="color: var(--danger); font-weight: 600;">Denied on:</span> <?php echo e($h->action_date); ?><br>
                    <span style="color: var(--text-muted); font-size: 13px;">Denied by: <?php echo e($h->admin->name ?? 'Unknown'); ?></span>
                    <?php elseif($h->status == 'RETURNED'): ?>
                    <span style="color: var(--primary); font-weight: 600;">Returned on:</span> <?php echo e($h->action_date); ?><br>
                    <span style="color: var(--text-muted); font-size: 13px;">Processed by: <?php echo e($h->admin->name ?? 'Unknown'); ?></span>
                    <?php else: ?>
                    <span style="color: var(--warning);">Waiting for approval...</span>
                    <?php endif; ?>
                </td>
                <td>
                    <?php if(in_array($h->status, ['APPROVED', 'RETURNED', 'DENIED'])): ?>
                    <a href="<?php echo e(route('student.receipt', $h->id)); ?>" style="display:inline-block; padding:6px 14px; background:var(--primary); color:#fff; border-radius:6px; font-size:12px; font-weight:500; text-decoration:none; transition:all 0.2s;" onmouseover="this.style.background='var(--primary-hover)'" onmouseout="this.style.background='var(--primary)'">View Receipt</a>
                    <?php else: ?>
    ...
    <script>
        ...
        function fetchNotifs() {
            fetch('<?php echo e(url("student/notifications")); ?>')
                .then(r => r.json())
                .then(d => {
        ...
        function markRead(id) {
            fetch('<?php echo e(url("student/notifications")); ?>/' + id + '/read', { method: 'POST', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' } })
                .then(() => { notifOpen = false; document.getElementById('notif-dropdown').style.display = 'none'; fetchNotifs(); });
        }
        function markAllRead() {
            fetch('<?php echo e(url("student/notifications/read-all")); ?>', { method: 'POST', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' } })
                .then(() => { fetchNotifs(); });
        }
...
<?php /**PATH C:\xampp\htdocs\LBS\resources\views/student/dashboard.blade.php ENDPATH**/ ?>